...
/*
There are a couple of problems in this class.Firstly, in the method of functionToPolygon,
the values of max and min did not be computed, and its proportion is not be computed to
suit the scene.At last, it needs another two point to connect the whole picture. The two
points’ location are in the beginning of X-axis and the end of X-axis.In the method of
functionToPolyline, its proportion is not computed to suit the scene.And the values of
max and min did not be computed.In the method of drawXAxis, the condition in if is wrong.
In the method of drawYAxis, the condition in if is wrong and the method of make line is wrong.
the line’s location is wrong.
Besides,this class has no method of computing of max and min.
 */
public class DisplayFunctionArea extends Application{
   ...
   private static double b;
     ...
     *   granularity n.
     */
    public static Polygon functionToPolygon(Function<Double,Double> f,//it need compute the value of max and min
        ...
        if (a >= b) {
            ...
            points[2*(n+1)] =X_SIZE;
            points[2*(n+1)+1] =max/(max-min)*Y_SIZE;//add four points
            points[2*(n+1)+2] =0;
            points[2*(n+1)+3] =max/(max-min)*Y_SIZE;
            /* Draw graph */
     ...
     */
    public static double max(Function<Double,Double> f, int n,//method of computing max
                             double a, double b) {
        double x, y;
    ...
    public static double min(Function<Double,Double> f, int n,//method of computing min
                             double a, double b) {
        double x, y;
    ...
    public static Polyline functionToPolyline(Function<Double,Double> f,
                                    int n, double a, double b) {//add the value of max and min and set the line
        if (a >= b) {
            throw new IllegalArgumentException();
        } else {
     ...
     */
    public static void drawXAxis(Group root){//the condition is wrong
     ...
     */
    public static void drawYAxis(Group root) {//the condition is wrong and the location is wrong.
        if (a <= 0 && b >= 0 && a != b){
